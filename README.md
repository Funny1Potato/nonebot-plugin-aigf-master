...
AIGFM_INCOMPLETE_TIMEOUT=40.0           # 消息可能不完整时的等待时间/秒（默认 40.0；同时是单次 VLM 识图的总超时）
...
- 纯 @ 消息或检测到同一用户连续发送时，以及调用的插件返回图片时，等待时间会延长至 `AIGFM_INCOMPLETE_TIMEOUT`（默认 40 秒）
- **有图片正在解析时整批推迟触发**：等到 VLM 描述填好后随**同一批**发给 LLM，不会把没有描述的空消息发出去；最长等待 `AIGFM_BATCH_TIMEOUT + AIGFM_INCOMPLETE_TIMEOUT`（默认 70 秒）后仍会放行，此时该图片以 `[图片解析中]` 进入上下文
...
- 工具返回「命令已投递」只代表事件已发出，**不代表执行成功**；只有后续聊天记录里出现 `[插件名]` / `[bot名]` 的响应才算成功
