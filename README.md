...
- 🔔 **群系统通知**：戳一戳、禁言/解禁、新人进群、退群被踢、消息撤回等会以 `[系统]` 消息进入 LLM 上下文（OneBot V11 文案最细，其它适配器按通知类别映射成中文，见「多适配器支持」）
...
- 每次处理时，LLM 收到最近 **20 条**（可配置）历史聊天记录 + 四层记忆 + 预设 + 表情包列表 + 其它插件响应；本批消息只出现在「新消息」段，同一条插件响应也只注入一次（不会在历史与新消息里重复）。历史聊天记录会落盘（`memory/<群号>/recent.json`），**机器人重启后仍能读回**
...
- 会话以 **`适配器:会话id`** 为键，例如 `onebot11:123456789`（群）、`console:console-chat`（频道）、`onebot11:20001`（私聊）
- `AIGFM_ENABLED_GROUPS` 管群/频道会话、`AIGFM_ENABLED_PRIVATE` 管私聊会话；条目可写带前缀的完整键，也可以只写 id（**按 OneBot V11 识别**，旧配置无需改动）
- 落盘目录名不能含 `:`，因此磁盘上写作 `onebot11_123456789`；`memory/` 下旧版裸数字目录会在启动时**一次性自动迁移**（目标已存在则跳过，幂等）
...
图例：**✅ 已实测通过** ｜ **⚠️ 已实现但未验证通过**（离线构造不出该适配器的事件，或需要真实 API / 真机）｜ **❌ 该适配器没有这类事件，或库层无法把该通知定位到群/频道会话（通知会被跳过）** ｜ **未测** 本次未构造该场景。

**已知特例**

- **@ 按昵称**：只有 OneBot V11 能按群昵称反查用户 id，其余适配器请让 LLM 直接用用户 id
- **插件调用**：Satori / Kaiheila 的事件结构特殊，离线未验证通过；其余 14 个已实测目标响应器收到命令
- **群通知**：为每个适配器构造真实通知事件实测，**10 个通过**（OneBot V11/V12、Satori、Telegram、QQ、Milky、Mirai、Kaiheila、DoDo、YunHu）；Discord / bilibili 未验证（离线 mock 不够 / 只有部分通知类在库的覆盖范围内）；**Feishu、Kritor、EFChat** 的通知 uninfo 与 alconna 都无法定位到群/频道会话，**Minecraft、WXMP** 的通知事件没有群/频道字段（会话按私聊处理）——这几种会被跳过（库层限制，非本插件可修）
...
上面四层记忆之外的**滚动窗口**：最近 50 条消息（含机器人自己的回复与插件响应）会落盘在 `<插件数据目录>/memory/<群号>/recent.json`，**机器人重启后会读回**，因此刚重启的会话也能接上之前的上下文（注入 prompt 的条数由 `AIGFM_RECENT_MESSAGES` 控制，默认 20）。`/reset` 会连这份记录一起清空。
