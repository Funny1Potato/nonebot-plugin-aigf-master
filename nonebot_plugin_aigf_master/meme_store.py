...
class MemeStore:
    ...
    def __init__(self, data_dir: Path, cache_dir: Path):
        ...
        self._recent_max = 5
        self._cache_index: dict[str, dict[str, dict]] = {}  # 会话键 -> {cache_id: 缓存信息}
    ...
    async def save_to_cache(self, session_key: str, image_bytes: bytes, description: str, emotion: str) -> str:
        ...
        bucket = self._cache_index.setdefault(session_key, {})
        bucket[cache_id] = {
            "path": str(cache_path), "description": description, "emotion": emotion,
        }
        # 索引常驻（不再随批处理清空），每会话条目数按上限丢最旧的
    ...
    def get_cached(self, session_key: str) -> list[dict]:
        return [
            {"id": cid, "description": info["description"], "emotion": info["emotion"]}
            for cid, info in self._cache_index.get(session_key, {}).items()
        ]

    def get_cache_info(self, session_key: str, cache_id: str) -> dict | None:
        return self._cache_index.get(session_key, {}).get(cache_id)
    ...
    def clear_cache(self, session_key: str):
        self._cache_index.pop(session_key, None)

    async def save_from_cache(self, session_key: str, cache_id: str, description: str, keywords: list[str]) -> bool:
        cache_info = self._cache_index.get(session_key, {}).get(cache_id)
        if not cache_info:
            return False
