...
若命令需要参数（如"决斗 @张三 10"），用 parts 参数按顺序给出参数段，结构与回复的 reply 字段相同：@ 群友用 `{{"type":"at","name":"群友昵称"}}`（昵称与聊天记录里的 `@昵称` 一致），普通文字用 `{{"type":"text","content":"文字"}}`；**各段之间的空格由系统拼接，不要自己加空格**。例：command 填 `决斗`、parts 填 `[{{"type":"at","name":"张三"}},{{"type":"text","content":"10"}}]`，实际发出「决斗 @张三 10」；不需要参数时省略。
