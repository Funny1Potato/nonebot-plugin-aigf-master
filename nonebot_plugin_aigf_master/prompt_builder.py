...
def build_prompt(
    ...
    if config.aigfm_meme_enabled:
        meme_judge_section = """
## 表情包收藏判断
消息中的图片都带 `[图片, id: xxx]`（含 VLM 描述；标记为"可能是表情包"的图片附带了情感）。结合上下文判断是否值得收藏为表情包：
- **可以收藏**：图片能表达一定的情感、适合在群聊中反复使用，且表情包库内没有类似图片，且值得保存——在 `memory.save_meme` 中填入 id、简短描述和关键词（这是 JSON 输出字段，不是命令）
- **不要收藏**：只是群友分享的照片、截图、梗图、普通图片，或表情包库内已有类似图片
- 若群友**连续发了多张图片**，通常是在分享普通图片
- 若群友**只发了一张图片**，而前后均没有与这张图片相关的内容，或前后仅有对该图片的介绍或评论，通常是在分享普通图片
- 若群友发的图片内容**与之前的内容有关联**，或这张图片是在其它群友发言之后发出的，且**包含较为明显的情感**（开心、愤怒、疑惑等），则可能是表情包
...
若命令需要 @ 某位群友（如"决斗 @张三"），可用 at_user_id 参数指定要 @ 的群友 QQ 号（从"已知群友昵称"的 `名字(QQ:号)` 中取）；不需要 @ 时省略。
...
- **查询/浏览类工具（query_member_info、query_image_library）只在对话确实需要该信息时才调用**：用户询问群友资料、你想回忆图片库内容时；不要为了"确认"或"展示能力"调用，也不要连续发起无意义的查询
- **能直接回答就不要调用工具**；一次会话只调用必要的工具轮次，克制使用

## 图片与表情包的区别
- **表情包（meme）**：「可用的表情包」清单里已列出的（[id] 描述（适用场景）），用 reply 的 type: meme 发送；由群友/管理员收藏进表情包库
- **图片库（image）**：你自己通过 `save_image` 收藏的图（聊天中遇到的梗图/有意思的图），平时不在 prompt 中，需要时用 `query_image_library` 查询、`send_library_image` 发送
- **收藏别搞混**：save_meme → 存表情包库；save_image → 存图片库——两个是不同的库，id 不互通
- **发送记录标注**「[表情包: id]」/「[图片: id]」用于区分来源；修改描述时 `edit_image_meta` 的 type 参数按记录里的类型传
- **群友评价联动**：当群友对 bot 刚发送的图片/表情包发表评价（好笑/难看/像什么）时，根据评价调用 `edit_image_meta` 修改或补充该图片/表情包的描述（反映群友反馈），完成后可在回复中提及已根据评价更新
"""
...
{{
  ...
  "memory": {{
    ...
    "save_image": [{{ "id": "缓存图片id", "description": "完整描述（画面内容/风格/用途），收藏进你自己的图片库，平时不显示，需要时用 query_image_library 查询" }}],
    "culture": {{
